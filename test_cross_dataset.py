...
def parse_args():
    parser = argparse.ArgumentParser(description="Test a configurable backbone on a cross dataset")
    parser.add_argument("--config", type=str, default="configs/config.yaml", help="Path to config YAML")
    parser.add_argument("--checkpoint", type=str, default="checkpoints/best_model.pth", help="Checkpoint path")
    parser.add_argument("--output-csv", type=str, default="cross_dataset_predictions.csv", help="CSV output path")
...
def main():
    ...
    image_paths, labels, probs = run_predict(model, loader, device, desc="Test cross dataset")
    ...
    print(f"Cross Dataset Test | {format_metrics(metrics)}")
