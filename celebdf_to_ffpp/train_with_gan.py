...
def parse_args():
    parser = argparse.ArgumentParser(description="Train on CelebDF train split plus GAN data")
...
def build_loaders(config: Dict) -> Tuple[DataLoader, DataLoader]:
    ...
    celebdf_train_dataset = DeepfakeFrameDataset(
    ...
    train_dataset = ConcatDataset([celebdf_train_dataset, gan_train_dataset])
...
def main():
    ...
    save_dir = Path(config.get("gan_save_dir") or config.get("save_dir", "checkpoints_celebdf_to_ffpp"))
    checkpoint_name = str(config.get("gan_checkpoint_name", "best_celebdf_to_ffpp_with_gan.pth"))
    ...
    if resume_path:
        ...
        epochs_without_accuracy_improvement = int(
            checkpoint.get("epochs_without_improvement", epochs_without_accuracy_improvement)
        )
