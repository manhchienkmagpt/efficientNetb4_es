...
def parse_args():
    parser = argparse.ArgumentParser(description="Test a CelebDF-trained checkpoint on the cross dataset (FF++)")
    parser.add_argument("--config", type=str, default=str(DEFAULT_CONFIG_PATH), help="Path to config YAML")
    parser.add_argument("--checkpoint", type=str, required=True, help="Checkpoint path")
    parser.add_argument(
        "--output-csv",
        type=str,
        default="outputs/ffpp_cross_predictions.csv",
...
def main():
    ...
    image_paths, labels, probs = predict(model, loader, device, desc="Test FF++ cross dataset")
    ...
    print(f"FF++ Cross Dataset Test | {format_metrics(metrics)}")
